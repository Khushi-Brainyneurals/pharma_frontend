  ...
  return (
    <section className="space-y-4" aria-label="Editable Cover and BOM data">
      ...
      <div className="overflow-hidden rounded-panel border border-border bg-surface shadow-sm">
        <div className="border-b border-border px-4 py-3"><h2 className="text-h2 font-semibold">Bill of materials</h2><p className="mt-1 text-micro text-subdued">{data.ingredients.length} extracted rows. Only changed rows are sent when you save.</p></div>
        <div className="max-h-[calc(100vh-370px)] min-h-[520px] overflow-auto">
          <table className="w-full min-w-[1180px] border-collapse text-small">
            ...
            <tbody>{data.ingredients.map((ingredient, index) => { const row = draft.ingredients[index]; return <tr key={row.rowKey} className="border-b border-border/80 align-top hover:bg-muted/40"><td className="w-24 p-2"><input value={row.srNo} inputMode="numeric" disabled={disabled} className="bom-table-input w-20" aria-label={`Serial number row ${index + 1}`} onChange={(event) => updateRow(index, "srNo", event.target.value)} />{errors[`ingredient-${index}-sr`] ? <FieldError message={errors[`ingredient-${index}-sr`]} /> : null}</td><td className="px-3 py-3 font-mono text-mono-sm text-subdued">{ingredient.material_code || "—"}</td><td className="min-w-[280px] p-2"><input value={row.ingredientName} disabled={disabled} className="bom-table-input w-full" aria-label={`Ingredient name row ${index + 1}`} onChange={(event) => updateRow(index, "ingredientName", event.target.value)} />{errors[`ingredient-${index}-name`] ? <FieldError message={errors[`ingredient-${index}-name`]} /> : null}</td><td className="w-28 p-2"><input value={row.uom} disabled={disabled} className="bom-table-input w-24" aria-label={`UOM row ${index + 1}`} onChange={(event) => updateRow(index, "uom", event.target.value)} />{errors[`ingredient-${index}-uom`] ? <FieldError message={errors[`ingredient-${index}-uom`]} /> : null}</td><td className="px-3 py-3 text-subdued">{ingredient.layer || "—"}</td><td className="min-w-48 px-3 py-3 text-subdued">{ingredient.part || "—"}</td><td className="px-3 py-3 text-right tabular-nums">{formatQuantity(ingredient.qty_per_mfc_batch)}</td><td className="px-3 py-3 text-right tabular-nums">{formatQuantity(ingredient.qty_required_production)}</td></tr>; })}</tbody>
          </table>
        </div>
      </div>
    </section>
